...
for target = 1:2
    ...
    for T = 1:80 % T=test trial
        ...
        for ch = 1:size(meanHGP_train,1)
            ...
            nmHGPch = (squeeze(meanHGP(ch,:,:)) - mu*ones(80,size(meanHGP,3)))./(sigma*ones(80,size(meanHGP,3)));
            nmHGP(ch,:,:) = nmHGPch;
        end
       ...
       %% SVM classification
        ...
        % Linear SVM
        [trainedClassifier, valAcc] = trainSVM_binary(traindata);
    ...
    clearvars -except meanHGP Y chIFG chSM1 binstart binend PredictionAccuracy Pvalue_all PredictorWeights
end
